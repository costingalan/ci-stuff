...
# Version: 2.0
# License: Apache V2.0
# Description: Create statistics for compute-hyperv for a specific date. 
...
LOGS_FILE=/home/jenkins-slave/compute-hyperv-statistics.log

function send_mail {
  echo "Sending mail"
  mail -s "[CI] Compute hyper-v stats - $1" "cgalan@cloudbasesolutions.com" < mail.txt
}
...
function calc_fail_init {
  ...
  do 
    echo "http://64.119.130.115/compute-hyperv/${i/;//}" >> mail.txt;
  done
  echo >> mail.txt
}
