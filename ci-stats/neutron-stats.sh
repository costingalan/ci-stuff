...
# Version: 2.0
# License: Apache V2.0
# Description: Create statistics for neutron for a specific date. 
...
LOGS_FILE=/home/jenkins-slave/neutron-statistics.log

function send_mail {
  echo "Sending mail"
  mail -s "[CI] Neutron stats - $1" "cgalan@cloudbasesolutions.com" < mail.txt
}
...
function calc_fail_init {
  ...
  do 
    echo "http://64.119.130.115/neutron/${i/;//}" >> mail.txt;
  done
  echo >> mail.txt
}
